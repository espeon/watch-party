...
export default function Test() {
  ...
  useEffect(() => {
    if (player.current) {
      const renderer = new LibASSTextRenderer(() => import("jassub"), {
        ...
        offscreenRender: !isDesktopBrowser,
  ...
  return (
    <div className="max-w-screen max-h-screen h-full">
    <MediaPlayer
      ref={player}
      viewType="video"
      title="Sprite Fight"
      src="http://localhost:8000/out/3/main.m3u8"
      crossOrigin={true}
      className="max-h-screen h-full"
    >
      <div className="flex flex-1">
      <MediaProvider className="*:max-h-screen" style={{ display: "block" }}>
        <Track
          src="http://localhost:8000/out/3/sub/eng.ass"
          kind="subtitles"
          label="English"
          type="ass"
          default
